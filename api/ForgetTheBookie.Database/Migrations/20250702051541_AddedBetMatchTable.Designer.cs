// <auto-generated />
using System;
using ForgetTheBookie.Database;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace ForgetTheBookie.Database.Migrations
{
    [DbContext(typeof(DatabaseContext))]
    [Migration("20250702051541_AddedBetMatchTable")]
    partial class AddedBetMatchTable
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.6")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("ForgetTheBookie.Database.Model.BetMatch", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<Guid>("AcceptedByUserId")
                        .HasColumnType("uuid");

                    b.Property<decimal?>("BackerPayout")
                        .HasColumnType("numeric");

                    b.Property<string>("BetMatchResult")
                        .IsRequired()
                        .HasColumnType("character varying(20)");

                    b.Property<Guid>("BetOfferId")
                        .HasColumnType("uuid");

                    b.Property<decimal?>("LayerPayout")
                        .HasColumnType("numeric");

                    b.Property<decimal>("MatchedAmount")
                        .HasColumnType("numeric");

                    b.Property<decimal?>("RakeCollected")
                        .HasColumnType("numeric");

                    b.HasKey("Id");

                    b.HasIndex("AcceptedByUserId");

                    b.HasIndex("BetOfferId")
                        .IsUnique();

                    b.ToTable("BetMatch");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.BetOffer", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<Guid?>("AcceptedByUserId")
                        .HasColumnType("uuid");

                    b.Property<string>("BetSide")
                        .IsRequired()
                        .HasColumnType("character varying(20)");

                    b.Property<Guid>("EventId")
                        .HasColumnType("uuid");

                    b.Property<string>("Odds")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("ProposedByUserId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Stake")
                        .HasColumnType("numeric");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("character varying(20)");

                    b.Property<string>("WinSelection")
                        .IsRequired()
                        .HasColumnType("character varying(20)");

                    b.HasKey("Id");

                    b.HasIndex("AcceptedByUserId");

                    b.HasIndex("EventId");

                    b.HasIndex("ProposedByUserId");

                    b.ToTable("BetOffer");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Event", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<DateTime?>("EndDateTime")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("FixtureId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("StartDateTime")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("Team1Id")
                        .HasColumnType("uuid");

                    b.Property<Guid>("Team2Id")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("Team1Id");

                    b.HasIndex("Team2Id");

                    b.ToTable("Event");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.EventSyncTrack", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.ToTable("EventSyncTrack");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.League", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<string>("CountryName")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<int>("ExternalId")
                        .HasColumnType("integer");

                    b.Property<bool>("IsCurrent")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<int>("SeasonId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("SeasonId");

                    b.ToTable("League");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Result", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<Guid>("EventId")
                        .HasColumnType("uuid");

                    b.Property<string>("EventResult")
                        .HasColumnType("character varying(20)");

                    b.Property<int?>("Team1Goals")
                        .HasColumnType("integer");

                    b.Property<int?>("Team2Goals")
                        .HasColumnType("integer");

                    b.Property<Guid?>("WinningTeamId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("EventId")
                        .IsUnique();

                    b.HasIndex("WinningTeamId");

                    b.ToTable("Result");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Season", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.HasKey("Id");

                    b.ToTable("Season");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.SeasonLeague", b =>
                {
                    b.Property<int>("SeasonId")
                        .HasColumnType("integer");

                    b.Property<Guid>("LeagueId")
                        .HasColumnType("uuid");

                    b.HasKey("SeasonId", "LeagueId");

                    b.HasIndex("LeagueId");

                    b.ToTable("SeasonLeague");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Team", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<int>("ExternalId")
                        .HasColumnType("integer");

                    b.Property<Guid?>("LeagueId")
                        .HasColumnType("uuid");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.HasKey("Id");

                    b.HasIndex("LeagueId");

                    b.ToTable("Team");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Transaction", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<decimal>("Amount")
                        .HasColumnType("numeric");

                    b.Property<Guid?>("BetOfferId")
                        .HasColumnType("uuid");

                    b.Property<decimal?>("RunningBalance")
                        .HasColumnType("numeric");

                    b.Property<DateTime>("TransactionDateTime")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("character varying(20)");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("BetOfferId");

                    b.HasIndex("UserId");

                    b.ToTable("Transaction");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.User", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("integer");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("DateOfBirth")
                        .HasColumnType("date");

                    b.Property<string>("Email")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("boolean");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("boolean");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("text");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("text");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("boolean");

                    b.Property<string>("RefreshToken")
                        .HasColumnType("text");

                    b.Property<DateTime>("RefreshTokenExpiry")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("text");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("boolean");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex");

                    b.ToTable("User", (string)null);
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.UserBalance", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<decimal>("Balance")
                        .HasColumnType("numeric");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("UserBalance");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<System.Guid>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("text");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<System.Guid>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("text");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("text");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<System.Guid>", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.Property<string>("Value")
                        .HasColumnType("text");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.BetMatch", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.User", "AcceptedByUser")
                        .WithMany("BetMatches")
                        .HasForeignKey("AcceptedByUserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ForgetTheBookie.Database.Model.BetOffer", "BetOffer")
                        .WithOne("BetMatch")
                        .HasForeignKey("ForgetTheBookie.Database.Model.BetMatch", "BetOfferId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("AcceptedByUser");

                    b.Navigation("BetOffer");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.BetOffer", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.User", "AcceptedByUser")
                        .WithMany("AcceptedBets")
                        .HasForeignKey("AcceptedByUserId");

                    b.HasOne("ForgetTheBookie.Database.Model.Event", "Event")
                        .WithMany("BetOffers")
                        .HasForeignKey("EventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ForgetTheBookie.Database.Model.User", "ProposedByUser")
                        .WithMany("BetOffers")
                        .HasForeignKey("ProposedByUserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("AcceptedByUser");

                    b.Navigation("Event");

                    b.Navigation("ProposedByUser");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Event", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.Team", "Team1")
                        .WithMany("EventsAsTeam1")
                        .HasForeignKey("Team1Id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ForgetTheBookie.Database.Model.Team", "Team2")
                        .WithMany("EventsAsTeam2")
                        .HasForeignKey("Team2Id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Team1");

                    b.Navigation("Team2");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.League", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.Season", "Season")
                        .WithMany()
                        .HasForeignKey("SeasonId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Season");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Result", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.Event", "Event")
                        .WithOne("Result")
                        .HasForeignKey("ForgetTheBookie.Database.Model.Result", "EventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ForgetTheBookie.Database.Model.Team", "Team")
                        .WithMany("Results")
                        .HasForeignKey("WinningTeamId");

                    b.Navigation("Event");

                    b.Navigation("Team");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.SeasonLeague", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.League", "League")
                        .WithMany("SeasonLeagues")
                        .HasForeignKey("LeagueId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ForgetTheBookie.Database.Model.Season", "Season")
                        .WithMany("SeasonLeagues")
                        .HasForeignKey("SeasonId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("League");

                    b.Navigation("Season");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Team", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.League", "League")
                        .WithMany("Teams")
                        .HasForeignKey("LeagueId");

                    b.Navigation("League");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Transaction", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.BetOffer", "BetOffer")
                        .WithMany("Transactions")
                        .HasForeignKey("BetOfferId");

                    b.HasOne("ForgetTheBookie.Database.Model.User", "User")
                        .WithMany("Transactions")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BetOffer");

                    b.Navigation("User");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.UserBalance", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.User", "User")
                        .WithOne("UserBalance")
                        .HasForeignKey("ForgetTheBookie.Database.Model.UserBalance", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<System.Guid>", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<System.Guid>", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<System.Guid>", b =>
                {
                    b.HasOne("ForgetTheBookie.Database.Model.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.BetOffer", b =>
                {
                    b.Navigation("BetMatch")
                        .IsRequired();

                    b.Navigation("Transactions");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Event", b =>
                {
                    b.Navigation("BetOffers");

                    b.Navigation("Result")
                        .IsRequired();
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.League", b =>
                {
                    b.Navigation("SeasonLeagues");

                    b.Navigation("Teams");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Season", b =>
                {
                    b.Navigation("SeasonLeagues");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.Team", b =>
                {
                    b.Navigation("EventsAsTeam1");

                    b.Navigation("EventsAsTeam2");

                    b.Navigation("Results");
                });

            modelBuilder.Entity("ForgetTheBookie.Database.Model.User", b =>
                {
                    b.Navigation("AcceptedBets");

                    b.Navigation("BetMatches");

                    b.Navigation("BetOffers");

                    b.Navigation("Transactions");

                    b.Navigation("UserBalance")
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
